...
<div class='titleText titleText27'><span>会员登录</span></div>
</td>
...
<td class='msgText'></td>
    ...
    UserPayInfo order=(UserPayInfo)session.getAttribute("order");
    ...
    <table cellpadding="10" cellspacing="0">
        <tr>
            <td colspan="3" align="center" class="td6">收货信息</td>
        </tr>
        <tr>
            <td class="td2">收货人：</td>
            <td colspan="2" class="td5"><%=order.getName()%></td>
        </tr>
        <tr>
            <td class="td2">收货地址：</td>
            <td colspan="2" class="td5"><%=order.getAddress() %></td>

        </tr>
        <tr>
            <td class="td2">联系电话：</td>
            <td colspan="2" class="td5"><%=order.getPhone() %></td>
        </tr>
        <tr>
            <td colspan="3" align="center" class="td6">支付信息</td>
        </tr>
        <tr>
            <td class="td2">支付类别：</td>
            <td colspan="2" class="td5"><%=order.getType() %></td>
        </tr>
        <tr>
            <td class="td2">账号：</td>
            <td colspan="2" class="td5"><%=order.getAccount() %></td>
        </tr>
         <tr>
            <td colspan="3" align="center" class="td6">商品信息</td>
        </tr>
        <tr>
        	<td class="td2">商品名</td><td class="td8">单价</td><td class="td5">购买量</td>
        </tr>
        <%
        sum=0;
        for(int i=0;i<order.getBookInfos().size();i++){
        	out.println("<tr><td  class=td2>"+order.getBookInfos().get(i).getBook().getName()+"</td><td <td class=td8>"+order.getBookInfos().get(i).getBook().getPrice()+"</td><td class=td5>"+order.getBookInfos().get(i).getQuantities()+"</td></tr>");
       		sum+=order.getBookInfos().get(i).getBook().getPrice()*order.getBookInfos().get(i).getQuantities();
        }
        %>
        
        <tr><td  colspan="3" class="td9">总价格</td></tr>
        <tr>
	        <td  colspan="3" class="td9">
	           <%=sum%>
	           </td>
        </tr>

                <tr>
            <td colspan="3" class="td3"><input type="submit" value="确认" id="checkbox2" class="chk_2" />
            确定</td>
        </tr>
    </table>
    </form>
	<form action="product?product=exit" method="POST">
		<td  class=td2>
		<input type="submit" value='退出账户'>
		</td>
	</form>
